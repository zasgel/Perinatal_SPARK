# =====================================================
# PRS ~ Perinatal Outcomes — POPULATION-LEVEL, ASD COVARIATE — SPARK V20
# =====================================================
...
OUT_DIR <- file.path(PC_DIR, "population_regression_ASD_covariate_V20")
...
fit_one_ancestry <- function(df_anc, anc_label) {
  ...
  covar_rhs <- paste(c("asd", "sex", "batch", pc_names), collapse = " + ")
...
anc_files <- c(
  ...
  African            = file.path(OUT_DIR, "PRS_outcome_African.tsv"),
  `Admixed American` = file.path(OUT_DIR, "PRS_outcome_Admixed_American.tsv"))
anc_files <- anc_files[file.exists(anc_files)]

res_all <- imap_dfr(anc_files, function(path, anc)
  read_tsv(path, show_col_types = FALSE) %>% mutate(ancestry = anc))

cat("\n===== Sample: probands + siblings | ASD covariate | cluster-robust SEs (", CLUSTER_TYPE, ") on family ID =====\n")
...
cat("\nFDR-significant results (BH q < 0.05), by ancestry:\n")
sig <- res_all %>%
  filter(!is.na(q_BH), q_BH < 0.05) %>%
  mutate(outcome_label = recode(outcome, !!!outcome_map, .default = outcome)) %>%
  select(ancestry, trait, outcome_label, OR, CI_low, CI_high, p_value, q_BH) %>%
  arrange(ancestry, trait, outcome_label)
if (nrow(sig)) print(as.data.frame(sig)) else cat("  none\n")

cat("\nMeta-analysis, FDR-significant:\n")
print(as.data.frame(
  meta_stouffer %>%
    filter(q_BH_meta < 0.05) %>%
    arrange(trait, outcome_label) %>%
