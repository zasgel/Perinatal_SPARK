# =====================================================
# PRS ~ Perinatal Outcomes — POPULATION-LEVEL, PROBANDS + SIBLINGS — SPARK V20
# =====================================================
...
OUT_DIR <- file.path(PC_DIR, "population_regression_all")
...
# No ASD restriction in the primary population analysis.
meta_cov <- indiv %>%
  filter(IID %in% child_ids, !is.na(FID)) %>%
  distinct(IID, .keep_all = TRUE)

message("V20 analytic child sample: ", nrow(meta_cov),
...
# Sequencing batch comes from the iWES v3 genomic metadata.
...
fit_one_ancestry <- function(df_anc, anc_label) {
  ...
  fit_logistic <- function(outcome, prs) {
    ...
    # cluster-robust SEs on current V20 family ID (mandatory)
    ct <- tryCatch(
      lmtest::coeftest(
        fit,
...
# -----------------------------
cat("
===== SPARK V20 sample: probands + siblings with PRS | family-clustered HC1 SEs =====
")
