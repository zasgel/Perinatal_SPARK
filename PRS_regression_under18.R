# =====================================================
# PRS ~ Perinatal Outcomes — POPULATION-LEVEL, AGE ≤18 AT REGISTRATION — SPARK V20
# =====================================================
...
OUT_DIR <- file.path(PC_DIR, "population_regression_under18")
...
# -----------------------------
# V20 explicit child sample + age restriction
# -----------------------------
blank_na <- function(x) {
  x <- trimws(as.character(x))
...
# ≤18 years means age at registration <= 216 months, inclusive.
meta_cov <- indiv %>%
  filter(
    IID %in% child_ids,
    !is.na(FID),
...
message("V20 analytic child sample age ≤18 at registration: ", nrow(meta_cov),
        ...
        " | unknown ASD status: ", sum(is.na(meta_cov$asd)))
...
# Sequencing/genotyping batch comes from the iWES v3 genomic metadata.
...
for (code in c("EUR", "AFR", "AMR")) {
  ...
  res_anc <- res_anc %>%
    ...
    ungroup() %>%
    mutate(outcome_label_sort = recode(outcome, !!!outcome_map, .default = outcome)) %>%
    arrange(trait, outcome_label_sort) %>%
...
# -----------------------------
cat("
===== Sample: probands + siblings age ≤18 at enrollment | cluster-robust SEs (", CLUSTER_TYPE, ") on family ID =====
")
