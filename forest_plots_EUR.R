...
# ---------- Paths ----------
eur_population_path <- "/Users/asgelz01/Downloads/PRS_all/population_regression_all/prs_perinatal_clustered_European.csv"

eur_within_path  <- "/Users/asgelz01/Downloads/PRS_all/within_sibs_results_per_ancestry_childonly/paper_ready_within_sibs_European.csv"
eur_between_path <- "/Users/asgelz01/Downloads/PRS_all/within_sibs_results_per_ancestry_childonly/paper_ready_between_sibs_European.csv"
...
for (i in seq_along(page_chunks)) {
  ...
  page_plots <- lapply(seq_along(tr_page), function(j) {
    tr <- tr_page[j]
    p  <- plots[[tr]]
    # Keep outcome/complication labels visible on every panel for readability.
    p
  })
