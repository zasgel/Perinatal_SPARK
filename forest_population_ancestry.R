...
# ---------- 2) Paths ----------
base_dir <- "/Users/asgelz01/Downloads/PRS_all/population_regression_all"

eur_path <- file.path(base_dir, "prs_perinatal_clustered_European.csv")
afr_path <- file.path(base_dir, "prs_perinatal_clustered_African.csv")
amr_path <- file.path(base_dir, "prs_perinatal_clustered_Admixed_American.csv")
...
make_trait_plot <- function(df_trait, title_str, x_pad = 0.10, x_min = 0.5, x_max = 3) {
  ...
  ggplot(df_trait, aes(x = OR, y = Outcome_disp, color = Ancestry, shape = Ancestry)) +
    ...
    # Mark estimates whose OR extends beyond the displayed x-axis range.
    # The point remains capped at the plotting boundary, while the arrow
    # makes clear that the true estimate lies farther to the right.
    geom_segment(
      data = df_trait %>% filter(OR > hi),
      aes(
        x = hi / 1.10,
        xend = hi,
        y = Outcome_disp,
    ...
    labs(
      title = title_str,
      x = "Odds ratio per 1-SD increase in PRS (log scale)",
      y = NULL
    ) +
...
traits <- c("Asthma", "Major depression", "Obesity", "Schizophrenia")
traits <- traits[traits %in% unique(dat$Trait_disp)]
...
for (tr in traits) {
  ...
  p <- make_trait_plot(df_tr, paste0(tr, " PRS"))
  plots[[tr]] <- p
...
for (i in seq_along(page_chunks)) {
  ...
  page_plots <- lapply(seq_along(tr_page), function(j) {
    tr <- tr_page[j]
    p  <- plots[[tr]]
    # Keep outcome labels visible on every panel.
p
  })
...
pdf(pdf_path, width = 18, height = 20)
for (p in panel_list) {
  print(p)
}
dev.off()
