###############################################################################
# Indirect genetic effects (duo model): perinatal ~ PRS_child * ASD_child + PRS_mother
###############################################################################
...
## ---------------------------------------------------------------------------
## 2. V20 offspring IDs, biological mother, ASD status, sex, batch
## ---------------------------------------------------------------------------
...
missing_sibling_ids <- setdiff(
  unique(c(affected_sibling_ids, control_sibling_ids)), indiv$spid
)
...
meta_genomic <- data.table::fread(META_PATH, sep = "\t", quote = "", header = TRUE) %>%
  ...
  transmute(
    ...
    twin_partner = dplyr::if_else(
      stringr::str_detect(twins_raw, "^SP\\d+$"),
...
med <- med %>%
  transmute(
    spid = as.character(.data[[id_col_med]]),
...
fit_duo <- function(dat, outc) {
  ...
  dat2 <- dat %>%
    filter(!is.na(.data[[outc]]), !is.na(asd))
  ...
  base <- tibble(
    ...
    n_asd = sum(dat2$asd == 1L),
    n_unaffected = sum(dat2$asd == 0L)
  )
  ...
  cells <- table(
    factor(dat2[[outc]], levels = c(0, 1)),
    factor(dat2$asd, levels = c(0, 1))
  )

  if (any(cells < MIN_EVENTS)) {
    message(
      "    EXCLUDED (sparse outcome x ASD cell): ", outc,
      " | cells: ",
      paste(
        sprintf(
          "y%s/asd%s=%d",
    ...
    return(base %>% mutate(status = "skipped_sparse_outcome_x_asd"))
  }
  ...
  ct <- tryCatch(
    lmtest::coeftest(
      m,
      vcov = sandwich::vcovCL(m, cluster = dat2$mother, type = "HC1")
    ),
    error = function(e) NULL
  )
  ...
  if (nrow(rows) == 0)
    return(base %>% mutate(status = "reported_terms_missing"))
