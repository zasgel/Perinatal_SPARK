###############################################################################
# Indirect genetic effects (duo model): perinatal ~ PRS_child + PRS_mother
###############################################################################
...
RESULTS_DIR <- file.path("/Users/asgelz01/Downloads/PRS_all", "ige_duo_results")
dir.create(RESULTS_DIR, showWarnings = FALSE, recursive = TRUE)
OUT_PATH <- file.path(RESULTS_DIR, "ige_duo_results.tsv")

## ---------------------------------------------------------------------------
## 1. PRS: load all .sscore files, parse trait + ancestry from filename
##    Patterns: {TRAIT}_{AFR|AMR}.sscore  and  EUR_{TRAIT}.sscore
## ---------------------------------------------------------------------------
...
# roles.csv is used only to identify the explicit offspring sample.
# Biological mother is NOT propagated from the proband row to sibling pointers.
# Each offspring's own biomother_sp_id is taken directly from
# individuals_registration.csv below.
...
# Legacy genomic metadata remains the source for batch and MZ annotation.
...
message("...with biological mother identified: ", sum(!is.na(ped$mother)))

## ---------------------------------------------------------------------------
## 3. Perinatal outcomes (parent-report about the child)
##    NA -> 0 recode applies to CHILDREN with a screening record, matching
##    the existing pipeline. Children with no record at all are excluded.
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 4. PCs per ancestry (cover parents and children; computed on step4_QC)
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 5. Build duos: child rows joined to mother's PRS, same ancestry required
## ---------------------------------------------------------------------------

# Keep one child per MZ twin pair (identical genotypes -> duplicated duos),
# matching the existing pipeline's keep_one_per_mz_pair convention.
children <- ped %>%
  filter(!is.na(mother)) %>%
...
residualize_prs <- function(ids, anc, trait_name) {
  # one row per person: a mother of k children must appear ONCE here, both so
  # the lookup joins 1:1 onto children and so multiparous mothers don't get
  # over-weighted in the residualization fit / standardization
...
results <- purrr::map_dfr(TARGET_ANCES, function(anc) {
  ...
  purrr::map_dfr(prs_traits, function(tr) {
    ...
    # each child must appear exactly once (inner_join can drop children
    # missing PRS, but must never duplicate them)
...
## ---------------------------------------------------------------------------
## 8. Stouffer meta across ancestries (N_eff weights), per trait x outcome
##    x coefficient — matching run_stouffer conventions
##    BH correction is applied separately by ancestry x trait x coefficient,
##    across outcomes.
## ---------------------------------------------------------------------------
...
## ---------------------------------------------------------------------------
## 9. PAPER-READY TABLES — same format as build_per_ancestry_paper_table in
##    the main pipeline: formatted OR [CI], Display string, BH flag.
##    One file per ancestry + META, both coefficients stacked as Term rows.
## ---------------------------------------------------------------------------
...
# Ancestry-specific paper tables: OR, 95% CI, p-value, and BH q-value.
# Z statistics are intentionally not shown here.
for (anc in TARGET_ANCES) {
  sub <- out %>% filter(ancestry == anc, status == "ok")
  if (nrow(sub) == 0) next

  fp <- file.path(RESULTS_DIR, paste0("ige_duo_paper_table_", anc, ".tsv"))
  data.table::fwrite(build_paper_table(sub), fp, sep = "\t")
  message("Wrote paper table: ", fp)
}

# META paper table: Meta Z is reported only here.
...
if (nrow(meta_sub) > 0) {
  ...
  meta_fp <- file.path(RESULTS_DIR, "ige_duo_paper_table_META.tsv")
