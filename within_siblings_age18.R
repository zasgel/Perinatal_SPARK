# =========================================================
# Within-siblings PRS analysis — AGE ≤18 AT REGISTRATION — Per ancestry (EUR, AFR, AMR) (age sensitivity)
# =========================================================
...
OUT_DIR <- "/Users/asgelz01/Downloads/PRS_all/within_sibs_results_age18_or_younger"
...
# Link each individual to the co-twin SPID(s) listed for them and resolve
# connected components (union-find; handles triplets+ and one-directional
# listings, i.e. only one twin lists the other).
...
meta <- indiv %>%
  filter(
    ...
    !is.na(FID),
    !is.na(sex),
    !is.na(age_at_registration_months),
    age_at_registration_months <= 216
  ) %>%
  inner_join(genomic_cov, by = "IID") %>%
  select(IID, FID, sex, batch, age_at_registration_months, mz_flag, mz_group) %>%
  distinct(IID, .keep_all = TRUE)

message("V20 age <=18 at registration proband/sibling sample: ", nrow(meta),
        " individuals in ", n_distinct(meta$FID), " families.")
...
# --- Primary MZ dedup: keep one member per (FID, mz_group) set.
#     Deterministic: keeps the lexicographically smallest SPID.
...
fit_one_condition <- function(yvar, d) {
  ...
  # Strict < so MIN_EVENTS = 5 means "at least 5"; non-events checked too
  # (a model with 300 cases / 3 controls must be excluded).
  if (events < MIN_EVENTS || nonevents < MIN_EVENTS)
    return(empty(paste0("Too few events or non-events (< ", MIN_EVENTS, ")")))
  # NEW: events among discordant families identify PRS_within — require
  # at least MIN_EVENTS there too (this check existed in the ASD papertables
  # script but was missing here).
  ...
  # Stability guard applies to the reported PRS terms, not nuisance covariates.
...
for (code in c("EUR", "AFR", "AMR")) {
  ...
  out_path <- file.path(OUT_DIR, paste0("within_sibs_age18_or_younger_", gsub(" ", "_", anc_label), ".tsv"))
  ...
  for (term_keep in c("PRS_within", "PRS_mean")) {
    ...
    paper_path <- file.path(
      OUT_DIR,
      paste0("paper_ready_", paper_tag, "_sibs_age18_or_younger_",
...
fwrite(
  res_meta_input,
  file.path(OUT_DIR, "stouffer_within_sibs_age18_or_younger_meta_input.tsv"),
  sep = "\t"
)
...
run_stouffer <- function(df_term, term_label) {
  ...
  out <- if (BH_SCOPE == "within_trait") {
    out %>% group_by(trait_full) %>%
      mutate(q_BH_meta = p.adjust(p_value_meta, method = "BH")) %>% ungroup()
  } else {
    out %>% mutate(q_BH_meta = p.adjust(p_value_meta, method = "BH"))
  }

  out %>% arrange(trait_full, outcome_label)
}
...
write_csv(
  meta_within,
  file.path(OUT_DIR, "stouffer_meta_within_family_age18_or_younger.csv")
)

write_csv(
  meta_between,
  file.path(OUT_DIR, "stouffer_meta_between_family_age18_or_younger.csv")
)
...
write_csv(
  meta_within_paper,
  file.path(OUT_DIR, "paper_ready_stouffer_meta_within_family_age18_or_younger.csv")
)
write_csv(
  meta_between_paper,
  file.path(OUT_DIR, "paper_ready_stouffer_meta_between_family_age18_or_younger.csv")
)
...
cat("\n===== V20 proband/sibling age <=18 at registration sample | MZ_MODE:", MZ_MODE,
    "| BH_SCOPE:", BH_SCOPE, "=====\n")

cat("\nSample per ancestry (fitted models):\n")
print(as.data.frame(
...
cat("\nFDR-significant (q<0.05) per ancestry x term:\n")
print(as.data.frame(
  res_all_summary %>% filter(!is.na(estimate)) %>%
    group_by(ancestry, term) %>%
    summarise(fitted = n(),
...
print(as.data.frame(
  res_all_summary %>% filter(is.na(estimate)) %>% count(ancestry, note, sort = TRUE)))

fwrite(res_all_summary,
       file.path(OUT_DIR, "within_sibs_age18_or_younger_all_ancestries.tsv"), sep = "\t")
message("\nWrote combined results to: ", OUT_DIR)
