...
OUT_DIR <- paste0("/Users/asgelz01/Downloads/PRS_all/within_sibs_results_per_ancestry_",
                  if (CHILD_ONLY) "childonly" else "allmembers")
...
# anyone named as a parent by anyone else
parent_ids <- unique(c(meta_raw$mother, meta_raw$father))
parent_ids <- parent_ids[!is.na(parent_ids)]

meta <- meta_raw %>%
  mutate(
    has_parent  = !is.na(mother) | !is.na(father),
    is_a_parent = spid %in% parent_ids,
    is_child    = has_parent & !is_a_parent,
    identical_twins = !(identical_twins %in% c(NA, "", "False", "0", "NA")),
    sex   = factor(sex),
    batch = factor(batch)
  ) %>%
  rename(IID = spid, FID = sfid) %>%
  select(IID, FID, father, mother, sex, batch, identical_twins, is_child, is_a_parent)

message("Pedigree: ", nrow(meta), " individuals | listed as a parent: ",
        sum(meta$is_a_parent), " | classified as children: ", sum(meta$is_child))

if (CHILD_ONLY) {
  n0 <- nrow(meta)
  meta <- meta %>% filter(is_child)
  message("CHILD_ONLY = TRUE  -> dropped ", n0 - nrow(meta), "; ", nrow(meta), " retained")
} else {
  message("CHILD_ONLY = FALSE -> parents retained (original sample definition)")
}
...
# =========================================
# Sample-composition summary (for comparing CHILD_ONLY TRUE vs FALSE)
# =========================================
