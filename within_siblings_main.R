...
# + Stouffer meta-analysis across ancestries          (v2)
...
OUT_DIR <- "/Users/asgelz01/Downloads/PRS_all/within_sibs_results_per_ancestry_childonly"
...
# Genomic metadata remains the source for batch and legacy MZ information.
meta_raw <- fread(META_PATH, sep = "\t") %>%
  as_tibble() %>%
  mutate(spid = as.character(spid))

# -----------------------------
# MZ twin identification (legacy iWES genomic metadata)
# -----------------------------
...
meta <- indiv %>%
  filter(IID %in% child_ids, !is.na(FID), !is.na(sex)) %>%
  inner_join(genomic_cov, by = "IID") %>%
  select(IID, FID, sex, batch, mz_flag, mz_group) %>%
  distinct(IID, .keep_all = TRUE)

message("V20 explicit proband/sibling sample: ", nrow(meta),
        " individuals in ", n_distinct(meta$FID), " families.")

# -----------------------------
# V20 phenotype coding
# blank = 0; "1" = 1; na_survey_logic = missing/not administered
# -----------------------------
...
cat("\n===== V20 explicit proband/sibling sample | MZ_MODE:", MZ_MODE,
    "| BH_SCOPE:", BH_SCOPE, "=====\n")
...
fwrite(res_all_summary,
       file.path(OUT_DIR, "within_sibs_all_ancestries.tsv"), sep = "\t")
